...
class PagesQueryBuilder extends QueryBuilder {
    ...
    public function filterPublished($published) {
       ...
       switch($published){
            case 'published':
                return $this->andWhere($this->expr()->orx(
                            $this->expr()->lte('p.published', ':date_now'),
                            $this->expr()->isNull('p.published')))
                            ->setParameter('date_now',new \DateTime('now'));
            case 'awaiting':
                return $this->andWhere($this->expr()->gte('p.published', ':date_now'))
                            ->setParameter('date_now',new \DateTime('now'));                
            case 'unset':
                return $this->andWhere($this->expr()->isNull('p.published'));                           
            }  
    }
    ...
    public function filterExpired($expired) {
        ...
        switch($expired){
            case 'published':
                return $this->andWhere($this->expr()->orx(
                            $this->expr()->gte('p.expired', ':date_now'),
                            $this->expr()->isNull('p.expired')))
                            ->setParameter('date_now',new \DateTime('now'));
            case 'expired':
                return $this->andWhere($this->expr()->lte('p.expired', ':date_now'))
                            ->setParameter('date_now',new \DateTime('now'));
            case 'awaiting':
                return $this->andWhere($this->expr()->gte('p.expired', ':date_now'))
                            ->setParameter('date_now',new \DateTime('now'));
            case 'unset':
                return $this->andWhere($this->expr()->isNull('p.expired'));                
            }
    }
    ...
    public function filterShowinmenu($inmenu) {
        if ($inmenu !== false) {
            return $this
                            ->andWhere('p.inmenu = :inmenu')
                            ->setParameter('inmenu', $inmenu);
        }
    }
    ...
    public function filterDeleted($deletedAt) {
        if ($deletedAt === false){
            return $this->andWhere($this->expr()->isNull('p.deletedAt'));
        }        
    }
    ...
    public function addSearch($s) {
        ...
        switch ($search_field){
            ...
            case 'urltitle':
                    return $this
                            ->andWhere('p.urltitle LIKE :search')
                            ->setParameter('search', '%'.$search.'%');   
        }
